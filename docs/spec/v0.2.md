> **Archived.** This is The CLI Spec v0.2 exactly as it stood when it was
> frozen on 2026-08-13. It is kept here unchanged so that a tool claiming
> `"clispec": "0.2"` can always be read against the text it was written for.
> Nothing on this page will change again; see [Versions](../versions.md) for
> what that guarantee covers, and [the current spec](../index.md) for v0.3.
> The text is reproduced unaltered but for one link: the licence link was
> relative to the site root and is written here as an absolute URL so it
> resolves from this path.
...
|---|-----------|----------------|
| 1 | [Structured Output](#1-structured-output) | Always offer explicit JSON; prefer JSON when piped while preserving established text contracts. |
...
The format flag is three-valued: its default (`auto`) selects the format by TTY detection, and an explicit value always wins. `mytool -o text list | grep web` must produce text, not JSON. Do not make the human format the flag's default value - that makes "explicitly chose text" indistinguishable from "did not choose", and the explicit choice gets silently overridden when piped.

`--output`/`-o` is the canonical spelling. If a tool's `--output` or `-o` is already bound by an existing contract (the destination-path convention of curl and most compilers), `--format` is an acceptable alternative; repurposing the existing flag would break the contract the spec tells you to protect. Whichever flag the tool uses must be declared in the schema's `global_args` so consumers discover it instead of guessing. Consumers place global arguments before the command path (`mytool -o json list`), which works consistently across common CLI frameworks; tools MAY additionally accept them after the command path.
...
Command paths have two interoperable encodings, declared by the top-level `command_layout` field:

- **`flat`**: every entry is top-level, `name` is the complete space-separated path (`"list"`, `"sites use"`, `"files download"`), and `subcommands` is omitted.
- **`nested`**: each `name` is one path segment; consumers join ancestor names with spaces. A `sites` entry can therefore contain a `use` subcommand.

Producers MUST use one layout consistently and MUST NOT mix flat paths with nested `subcommands`. Consumers MUST accept both layouts. For documents created before `command_layout` existed, consumers infer `nested` if any entry contains `subcommands`, and `flat` otherwise. Producers SHOULD emit `command_layout` explicitly.

In nested layout, a group entry may exist only to route to children. `mutating` is meaningful only on an invocable command; consumers must not derive trust from a structural group, and producers must not set `mutating` on a group that cannot itself execute.
...
v0.2 extends v0.1 with additional optional contract fields and clarified `mutating` semantics. A v0.1 document that does not already use the newly reserved property names with incompatible shapes validates against v0.2 unchanged. [`v0.1.json`](/schema/v0.1.json) remains published.

Five widening amendments were folded into v0.2 shortly after publication: the `--format` spelling allowance for tools whose `--output` is already taken, the optional `outcomes` array, the optional per-command `example` invocation (a complete example command line, so a consumer can see one correct call without assembling it from the argument list), relaxing the piped-output default (a tool with an established human-text default may keep it, provided it declares the default in the top-level `output` field), and the optional `command_layout` discriminator that makes the existing flat and nested command encodings unambiguous. All five only relax, add, or clarify; every document and tool that conformed to v0.2 at publication still conforms.
...
$ mytool -o json list --limit 10 --offset 0
{"items": [...], "total": 1847, "limit": 10, "offset": 0}

$ mytool -o json list --limit 10 --offset 10
{"items": [...], "total": 1847, "limit": 10, "offset": 10}

$ mytool -o json list --fields name,status --limit 10
...
- [ ] Uses one command layout consistently and does not mix flat command paths with nested `subcommands`; new schemas SHOULD declare the layout with `command_layout`. *(Principle 2)*
